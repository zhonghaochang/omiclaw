...
import { MATCLAW_DIR, STATE_FILE } from './constants.js';
...
export async function applySkill(skillDir: string): Promise<ApplyResult> {
  ...
  const statePath = path.join(projectRoot, MATCLAW_DIR, STATE_FILE);
  ...
  for (const relPath of manifest.modifies) {
    ...
    const basePath = path.join(projectRoot, MATCLAW_DIR, 'base', resolvedPath);
  ...
  try {
    ...
    for (const relPath of manifest.modifies) {
      ...
      const tmpCurrent = path.join(
        os.tmpdir(),
        `matclaw-merge-${crypto.randomUUID()}-${path.basename(relPath)}`,
      );
