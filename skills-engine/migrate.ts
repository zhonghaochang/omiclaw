...
import { BASE_DIR, CUSTOM_DIR, MATCLAW_DIR } from './constants.js';
import { initNanoclawDir } from './init.js';
import { recordCustomModification } from './state.js';

export function initSkillsSystem(): void {
  initNanoclawDir();
  console.log('Skills system initialized. .matclaw/ directory created.');
}
...
export function migrateExisting(): void {
  ...
  try {
    ...
    if (diff.trim()) {
      ...
      console.log(
        'Custom modifications captured in .matclaw/custom/migration.patch',
      );
    } else {
