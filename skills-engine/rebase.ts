...
import { BASE_DIR, MATCLAW_DIR } from './constants.js';
...
export async function rebase(newBasePath?: string): Promise<RebaseResult> {
  ...
  try {
    ...
    const stateFilePath = path.join(projectRoot, MATCLAW_DIR, 'state.yaml');
    ...
    try {
      ...
      // Save combined patch
      const patchPath = path.join(projectRoot, MATCLAW_DIR, 'combined.patch');
      ...
      if (newBasePath) {
        ...
        for (const relPath of trackedFiles) {
          ...
          const tmpSaved = path.join(
            os.tmpdir(),
            `matclaw-rebase-${crypto.randomUUID()}-${path.basename(relPath)}`,
          );
