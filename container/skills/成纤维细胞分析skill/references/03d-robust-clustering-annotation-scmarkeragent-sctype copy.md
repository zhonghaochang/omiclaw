...
description: Section 3 鲁棒版独立规范：修复 Step3 resume 的 cluster-id 漂移、Step4 subtype 强制投射与低置信接受问题；要求 exact checkpoint resume、major purity gate、稳健 rearrangement/discard，以及 Section 4-ready annotated h5ad 交付。
type: reference
---
...
- `raw_label` 必须被永久保留并写入正式输出；`standard_label` 只能在通过置信门禁后正式采用
...
本规范的正式 dictionary / contract 资产一律从以下路径读取：
...
以下清洗/审计文件若存在，视为 03d dictionary bundle 的正式伴随审计物：
...
- `assets/dictionary/03d_upgrade_summary.tsv`
- `assets/dictionary/03d_upgrade_qc_report.tsv`

以下 bridge / database 资产仍可从原路径读取：
...
若仍检测到旧 `assets/03c_*` dictionary 路径，必须直接失败：`ERROR:old_dictionary_path_detected`。
...
- `raw_subtype_label_top1`
  来自 bridge/scType/scMarkerAgent 的原始 subtype 候选，必须保留
- `standard_subtype_candidate`
  由 stage-aware dictionary 投射得到的标准 subtype 候选，必须保留，即使最终不被接受
- `Cell_Subtype`
...
- 用 `03d_stage_aware_raw_to_standard_projection_dictionary.tsv` 做 stage-aware 投射
...
- fixed marker program recovery from bundled marker assets / in-script constant marker sets
...
- `composite_resolution_score`
  ...
  `0.30 * separation + 0.20 * homogeneity + 0.15 * marker_separation + 0.10 * cluster_size_balance + 0.15 * lineage_recovery + 0.10 * split_gain - 0.20 * underclustering_penalty`。
...
- 第四优先级：`lineage_recovery_score` 降序
- 第五优先级：`separation_score` 降序
- 第六优先级：`homogeneity_score` 降序
- 第七优先级：`marker_separation_score` 降序
- 最终 tie-break：若仍完全相同，选择不产生 tiny-cluster warning 的较高 resolution；不得再默认较低 resolution 优先

`align_fraction`、`mean_top1_score`、`top1_raw_label`、`Major_CellType`、`Cell_Subtype`、`TopLevel_Compartment`、`Epithelial_CNV_Label` 只能作为 QC/解释字段，禁止进入主 ranking score 或 tie-break。
...
- R bridge with `stage_03c = step4_{major}`
- 输出 `raw_subtype_label_top1` 与 `standard_subtype_candidate`
...
- override 命中必须写入 `projection_rule`，并在 cluster 输出中保留原始 `raw_subtype_label_top1`；禁止用 override 后的标准标签覆盖 raw label provenance
...
- 保留 `raw_subtype_label_top1`、`standard_subtype_candidate` 与失败原因
...
- `final_subtype`
- `raw_subtype_label_top1`
- `raw_subtype_label_top2`
- `raw_subtype_label_consensus`
- `standard_subtype_candidate`
- `projection_rule`
- `top1_score`
- `top2_score`
...
注意：正式列名必须使用 `raw_label`，不得把用户常见拼写错误 `raw_lable` 写入文件 schema；caption 中可注明 `raw_lable` 等同于 `raw_label`。
...
- `projection_rule`
- `raw_major_label_top1`
- `raw_major_label_top2`
- `raw_subtype_label_top1`
...
- `top1_raw_label`、`runner_up_raw_label`、`Subtype_Assignment_Score`、`projection_rule` 仍是 Section 3 的正式稳定导出列，不得被新列替代
- `Raw_Label_Primary` 必须与 `top1_raw_label` 完全一致；它的作用是把 `top / major / subtype / raw_label` 四层注释视图补齐为统一展示接口
- `Primary_Cell_Annotation` 必须与 `Cell_Subtype` 完全一致
- `Primary_Cell_Annotation_Level` 必须固定写为 `subtype`
- `raw_major_label_top1 / raw_major_label_top2 / raw_subtype_label_top1 / standard_* / *_status / section4_ready` 只能作为新增审计列或 handoff 列
...
- `work/figures/section3/subgroups/<major>_Subtype_UMAP_raw_label.pdf`
- `work/figures/section3/subgroups/<major>_Subtype_UMAP_raw_label.png`
- `work/figures/section3/subgroups/<major>_Subtype_UMAP_raw_label_source_data.tsv`
- `work/figures/section3/subgroups/<major>_Subtype_UMAP_raw_label_caption.md`
...
- `raw_label` 图按 `raw_subtype_label_top1` 或 cluster-level `raw_subtype_label_consensus` 着色，用于审计原始标签来源
- 三套图的 source data 都必须包含 `cell_id, UMAP1, UMAP2, Major_CellType, Cell_Subtype, projection_try_subtype, raw_subtype_label_top1, raw_subtype_label_consensus, section4_subtype_cluster_id`
...
- 是否仍存在双轨最终列
- 是否每个 searchable parent major 均输出 `<major>_subtype_cluster_mapping.tsv`，且包含 `raw_subtype_label_top1/raw_subtype_label_top2/raw_subtype_label_consensus/projection_try_subtype/final_subtype`
- 是否每个有细胞的 parent major 均输出正式 subtype 图、`projection_try` 图、`raw_label` 图三套 figure/source/caption
...
- ranking list 是否填充 `underclustering_penalty / split_gain_score / rare_lineage_recall_score / selected_reason`
...
- 用 `Major_CellType / Cell_Subtype / raw_label / TopLevel_Compartment / CNV label` 参与 resolution ranking 主分数或 tie-break
