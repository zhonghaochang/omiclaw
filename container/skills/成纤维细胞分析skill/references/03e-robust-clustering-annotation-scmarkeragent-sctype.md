...
Step3 bridge 可从任何 Step1 top-level 来源的 cluster 中产生 epithelial/cancer-lineage raw label（例如 Stroma 或 Review_queue 来源的 cluster，bridge 返回 "epithelial fibroblast cell" 等 raw label）。这类 Step3 bridge-derived epithelial candidate 也必须纳入 `03e_epithelial_cnv_candidate_closure.tsv`，并在 `closure_unit` 列中标记为 `step3_bridge_derived`。若候选 cluster 没有 Step2 CNV_Score（因为 Step1 非 epithelial），必须使用 marker-based fallback 进行 closure 判定，并记录 `cnv_score_available=False, epithelial_marker_support, parent_lineage_support`；若 marker-based fallback 也无法判定，不得直接写入 `Cancer_cell` 或 `Epithelial`，必须留在 `Review_queue` 或 parent lineage。
...
`Treg, Treg_Th17_like, TCF7_CD8, Cytotoxic_CD8, Exhausted_CD8, GammaDelta_T, NKT_like, MAIT_like, CD4_Naive, CD4_Memory, CD4_Activated, CD4_Cytotoxic, Th1, Th2, Th17, Tfh, Tissue_resident_T, Proliferating_T`

若 T_cell 最终全部为 `T_cell_unspecified`，必须 hard fail，除非 subtype search 真实执行并提供 marker-based 证据证明所有 T-cell child cluster 不稳定。

NK_cell subtype search 必须审计并优先保留稳定的：

`NK_CD56bright, NK_FCGR3A, NK_Resident, NK_Activated, NK_Exhausted, Proliferating_NK`

若 NK_cell 最终 50% 以上为 `NK_cell_unspecified` 或 `NKrest`，必须在 subtype acceptance audit 中解释原因，否则 hard fail：`ERROR:nk_subtype_overfragmented_unspecified`。

B_cell subtype search 必须审计并优先保留稳定的：

`Naive_B, Memory_B, Activated_B, Germinal_center_B, Atypical_memory_B, Regulatory_B, Plasma_B, Plasmablast, Proliferating_B`

Myeloid subtype search 必须审计并优先保留稳定的：

`CD14_Monocyte, FCGR3A_Monocyte, TAM_C1QC, M1_like, M2_like, cDC1, cDC2, pDC, mregDC, Mast, Neutrophil`
...
### Major-level 总 cluster UMAP 图（必须）

必须输出一张 major-level 全局 UMAP，展示所有细胞按 `Major_CellType` 着色：

- `work/figures/section3/Major_CellType_UMAP.pdf`
- `work/figures/section3/Major_CellType_UMAP.png`
- `work/figures/section3/Major_CellType_UMAP_source_data.tsv`
- `work/figures/section3/Major_CellType_UMAP_caption.md`

source data 必须包含：`cell_id, UMAP1, UMAP2, Major_CellType, TopLevel_Compartment, section3_major_cluster_id`

caption 必须写明：总细胞数、major cell type 数量与分布、selected resolution、是否包含 Epithelial/Cancer 的 CNV-lock cell。

若缺失该图，必须 hard fail：`ERROR:major_level_total_cluster_umap_missing`。

### Subtype-level 图件三件套（每个有细胞 parent major）

对每个有细胞 parent major 必须输出三张 UMAP：

1. **Final subtype label 图**：`<major>_Subtype_UMAP_final_label.pdf/png/source_data.tsv/caption.md`
   - source data：`cell_id, UMAP1, UMAP2, Major_CellType, Cell_Subtype, section4_subtype_cluster_id, annotation_confidence`
2. **Projection try 图**：`<major>_Subtype_UMAP_projection_try.pdf/png/source_data.tsv/caption.md`
   - source data：`cell_id, UMAP1, UMAP2, Major_CellType, standard_subtype_candidate, projection_rule`
3. **sctype rawoutput 图**：`<major>_Subtype_UMAP_sctype_rawoutput.pdf/png/source_data.tsv/caption.md`
   - source data：`cell_id, UMAP1, UMAP2, Major_CellType, Cell_Subtype, projection_try_subtype, sctype_rawoutput_top1, sctype_rawoutput_consensus, sctype_rawoutput_backend, section4_subtype_cluster_id`

### CNV density 图（必须）

- `CNV_Score_Continuous_Density.pdf/png/source_data.tsv/caption.md`
...
- Major-level 总 cluster UMAP 图缺失或 source_data 为空
...
- NK_cell 50% 以上 unspecified/NKrest 且无 subtype acceptance audit 解释
...
- [ ] Major-level 总 cluster UMAP 图已输出且 source_data 非空
- [ ] NK_cell subtype 分布中 unspecified/NKrest < 50% 或有充分 audit 解释
- [ ] B_cell subtype 分布已报告
...
- NK_cell subtype 分布（必须包含 NK_CD56bright, NK_FCGR3A, NK_Exhausted, NK_Resident, NK_Activated, Proliferating_NK, NKrest, NK_cell_unspecified 各自数量）
- T_cell subtype 分布
- B_cell subtype 分布（必须包含各 contract subtype 数量）
